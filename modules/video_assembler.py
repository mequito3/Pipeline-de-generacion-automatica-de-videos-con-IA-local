...
def generate_thumbnail(script: dict, images: list, output_path: str) -> str:
    """
    ...
    # Compatibilidad: aplanar list[list[str]] → list[str]
    if images and isinstance(images[0], list):
        images = [clips[0] for clips in images if clips]
    ...
    # Usar urgency del script si existe; sino lista amplia
    urgency = (script.get("thumbnail_text") or "").strip().upper()
    if not urgency:
        urgency_texts = [
            "MIRA ESTO", "HISTORIA IMPACTANTE", "NO PUEDO CREERLO", "DEBES VERLO",
            "ESTO ES REAL", "LO QUE PASÓ DESPUÉS", "NO LO VAS A CREER",
            "CUÉNTAME QUE PIENSAS", "ES PEOR DE LO QUE CREES", "ESTO SÍ PASÓ",
            "DIOS MÍO", "QUÉ ASCO", "CÓMO ES POSIBLE", "LO HIZO DE VERDAD",
            "NADIE LO SABÍA", "DOS AÑOS ASÍ", "Y YO SIN SABER", "TRAICIÓN REAL",
            "SECRETO DESTRUIDO", "TODO ERA MENTIRA", "IMPACTANTE",
        ]
...
_CONTENT_ID_MARKERS = ("fesliyan", "bensound", "incompetech")


def _pick_music() -> Path | None:
    """
    Elige aleatoriamente un archivo de música de assets/music/.
    Excluye archivos de Fesliyan/Bensound/Incompetech (registrados en Content ID).
    """
    music_dir = config.ASSETS_DIR / "music"
    all_tracks = list(music_dir.glob("*.mp3")) + list(music_dir.glob("*.wav"))
    safe = [t for t in all_tracks if not any(m in t.name.lower() for m in _CONTENT_ID_MARKERS)]
    risky = [t for t in all_tracks if t not in safe]

    if risky and not safe:
        logger.warning(
            f"AVISO: solo hay música con Content ID ({[t.name for t in risky]}). "
            "Usa /music en Telegram para descargar tracks CC0 seguros. "
            "Usando igual — el video podría ser reclamado por YouTube."
        )
        return random.choice(risky)

    if not safe:
        # Intentar auto-descarga de CC0 (sin Chrome, sin API key)
        try:
            from modules.youtube_audio_library import download_freepd_tracks
            logger.info("Carpeta de música vacía — descargando tracks CC0 desde FreePD...")
            download_freepd_tracks(n=5)
            all_tracks = list(music_dir.glob("*.mp3")) + list(music_dir.glob("*.wav"))
            safe = [t for t in all_tracks if not any(m in t.name.lower() for m in _CONTENT_ID_MARKERS)]
        except Exception as _me:
            logger.warning(f"Auto-descarga de música falló: {_me}")
        if not safe:
            return None
...
def assemble_video(
    ...
    # Compatibilidad: pexels_fetcher devuelve list[list[str]] (multi-cut), tomar primer clip
    if images and isinstance(images[0], list):
        images = [clips[0] for clips in images if clips]
    ...
    try:
        ...
        # Secuencial — 1 worker (BSOD prevention: más de 1 ffmpeg en paralelo calienta el CPU)
        errors = []
